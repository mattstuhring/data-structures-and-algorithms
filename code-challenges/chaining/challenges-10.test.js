...
const totalSum = (input) => {
  return input.reduce((total, curr) => {
    let t = curr.reduce((total2, curr2) => {
      total2 += curr2;
      return total2;
    }, 0);
...
let findMaleAndFemale = (data) => {
  return data.reduce((total, curr, idx) => {
    if (curr.gender === 'male' || curr.gender === 'female') {
      if (idx !== data.length - 1) {
        total = total + curr.name + ' and ';
      } else {
        total = total + curr.name;
      }
    }
